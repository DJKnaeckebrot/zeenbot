    ...
    async execute(interaction, client) {
        ...
        switch (Sub) {
            ...
            case "add": {
                ...
                if (Member.id === user.id) return Reply(interaction, "❌", `You can't warn yourself!`, true)
                if (guild.ownerId === Member.id) return Reply(interaction, "❌", `You can't warn the server's owner!`, true)
                ...
                EditReply(interaction, "✅", `${Member} has been warned for: **${reason}**`)
            ...
            case "remove": {
                ...
                if (!data) return EditReply(interaction, "❌", `The ID you provided is not a valid ID!`)

                const member = guild.members.cache.get(data.User)

                await data.delete()

                EditReply(interaction, "✅", `Removed \`1\` warning from **${member}**`)
...
function warnlistEmbed(pages, number, guild, Member, client) {
    ...
    for (let i = 0; i < pages.length; i += number) {
        ...
        const Embed = new EmbedBuilder()
            ...
            .setFooter({ text: 'Warnings by zeenbot' })
            .setTimestamp()
